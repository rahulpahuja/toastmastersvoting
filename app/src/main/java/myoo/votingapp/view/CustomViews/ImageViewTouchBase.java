package myoo.votingapp.view.CustomViews;

/**
 * Created by MA294214 on 4/4/2017.
 */
...
import myoo.votingapp.Model.RotateBitmap;
...
public abstract class ImageViewTouchBase extends ImageView {
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_BACK && getScale() > 1.0f) {
            ...
            // image, otherwise Back returns the getUsersListRef to the gallery.
            zoomTo(1.0f);
            return true;
        }
    ...
    // peopeleview's dimensions then center it (literally).  If the image
    // is scaled larger than the peopeleview and is translated out of peopeleview
    // then translate it back into peopeleview (i.e. eliminate black bars).
    ...
    protected void zoomIn(float rate) {
        ...
        if (getScale() >= mMaxZoom) {
            return;     // Don't let the getUsersListRef zoom into the molecular level.
        }
        if (mBitmapDisplayed.getBitmap() == null) {
            return;
        }
